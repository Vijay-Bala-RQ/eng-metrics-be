...
export default async function handler(req: any, res: any) {
  ...
  console.log("[collectMetrics] method:", req.method, "query:", JSON.stringify(req.query));

  try {
    const org: string = req.query.org || process.env.ADO_ORG || "";
    const rawAdoProject: string = req.query.adoProject || process.env.ADO_PROJECT || "";
    const adoProject: string = decodeURIComponent(rawAdoProject.replace(/\+/g, " "));
    const repoId: string = req.query.repoId || "";
    const pipelineId: string = req.query.pipelineId || process.env.ADO_PIPELINE || "";
    ...
    console.log(`[collectMetrics] repoSource=${repoSource} fromDate=${fromDate || "all"} toDate=${toDate || "now"}`);

    const dateOnly = (iso: string) => iso.split("T")[0];

    const dateFilterPR = fromDate ? `&searchCriteria.minTime=${fromDate}` : "";
    const dateFilterCommit = fromDate ? `&searchCriteria.fromDate=${fromDate}` : "";
    ...
    if (repoSource === "github") {
      return await handleGithubRepo(req, res, { docId, githubFullName, githubPat, pipelineId, org, adoProject, fromDate, toDate, headers: req });
    }

    if (!org || !adoProject) {
      return res.status(400).json({ error: `Missing params. org="${org}" adoProject="${adoProject}"` });
    }
    ...
    const base = `https://dev.azure.com/${org}/${encodeURIComponent(adoProject)}`;
        ...
        for (const team of teams.slice(0, 10)) {
          try {
            ...
            for (const m of (membersRes.data.value || [])) {
              ...
              const emailLocal = uniqueName.includes("@") ? uniqueName.split("@")[0] : shortName;
              for (const key of [displayName, displayName.toLowerCase(), uniqueName, shortName, emailLocal, emailLocal.toLowerCase()]) {
                if (key) gitNameToDisplay[key] = displayName;
              }
            }
          } catch (e) { }
        }
      } catch (e: any) { console.log("[collectMetrics] teams error:", e?.message); }
    }
    ...
    } catch (e: any) { console.log("[collectMetrics] alias load error:", e?.message); }

    let avgReviewTimeHours = 0, prCount = 0, avgCommentsPerPr = 0, avgPrPickupHours = 0, totalPrComments = 0;
    const prLog: any[] = [];
    ...
    if (repoId && repoId !== "none") {
      try {
        ...
        let totalHours = 0, totalComments = 0, totalPickupHours = 0, pickupCount = 0;
        for (const pr of prs) {
          ...
          totalHours += hrs;
          const author: string = resolveToDisplay(pr.createdBy?.displayName || "Unknown");
          if (!developerPrMap[author]) developerPrMap[author] = { prCount: 0, totalReviewHours: 0, comments: 0 };
          developerPrMap[author].prCount++;
          developerPrMap[author].totalReviewHours += hrs;
          let commentCount = 0, firstCommentTime: number | null = null;
          try {
            ...
            for (const t of (threadRes.data.value || [])) {
              ...
              const ct = new Date(firstComment?.publishedDate || t.publishedDate).getTime();
              if (!isNaN(ct) && firstComment?.author?.id !== pr.createdBy?.id) {
                if (firstCommentTime === null || ct < firstCommentTime) firstCommentTime = ct;
              }
            }
            totalComments += commentCount;
          } catch (e) { }
          if (firstCommentTime !== null) { totalPickupHours += (firstCommentTime - created) / 3600000; pickupCount++; }
          developerPrMap[author].comments += commentCount;
          prLog.push({
            id: pr.pullRequestId, title: pr.title, status: pr.status, author,
            reviewTimeHours: Math.round(hrs * 10) / 10, comments: commentCount,
            createdDate: pr.creationDate, closedDate: pr.closedDate,
            isDraft: pr.isDraft || false,
            targetBranch: pr.targetRefName?.replace("refs/heads/", "") || "",
          });
        }
        if (prCount > 0 && totalHours > 0) avgReviewTimeHours = Math.round((totalHours / prCount) * 10) / 10;
        if (prCount > 0) {
          avgCommentsPerPr = Math.round(totalComments / prCount);
          totalPrComments = totalComments;
        }
        if (pickupCount > 0) avgPrPickupHours = Math.round((totalPickupHours / pickupCount) * 10) / 10;
      } catch (e: any) { console.log("[collectMetrics] PR error:", e?.message); }
    }

    let totalCommits = 0, uniqueContributors = 0;
    ...
    if (repoId && repoId !== "none") {
      try {
        ...
        for (const c of commits) {
          const rawName: string = c.author?.name || c.committer?.name || "Unknown";
          const email: string = c.author?.email || c.committer?.email || "";
          const emailLocal = email ? email.split("@")[0] : "";
          const resolved = resolveToDisplay(rawName) || (emailLocal ? resolveToDisplay(emailLocal) : rawName);
          commitsByAuthor[resolved] = (commitsByAuthor[resolved] || 0) + 1;
        }
        uniqueContributors = Object.keys(commitsByAuthor).length;
      } catch (e: any) { console.log("[collectMetrics] commit error:", e?.message); }
    }

    let successRate = 0, avgBuildTime = 0, coverage: number | null = null;
    let longestBuildMins = 0, shortestBuildMins = Infinity;
    ...
    if (pipelineId && pipelineId !== "none") {
      try {
        ...
        for (const b of builds) {
          ...
          if (b.startTime && b.finishTime) {
            durationMins = (new Date(b.finishTime).getTime() - new Date(b.startTime).getTime()) / 60000;
            totalTime += durationMins;
            if (durationMins > longestBuildMins) longestBuildMins = durationMins;
            if (durationMins < shortestBuildMins) shortestBuildMins = durationMins;
          }
          buildLog.push({
            id: b.id, buildNumber: b.buildNumber, result, status: b.status,
            startTime: b.startTime, finishTime: b.finishTime,
        ...
        let totalTestMins = 0, testBuildCount = 0;
        ...
        for (let i = 0; i < buildLog.length; i++) {
          ...
          try {
            ...
            buildEntry.stages = records
              .filter((r: any) => r.type === "Stage" || r.type === "Phase" || r.type === "Job")
              .map((r: any) => ({
                name: r.name, state: r.state, result: r.result,
                durationSecs: r.startTime && r.finishTime
                  ? (new Date(r.finishTime).getTime() - new Date(r.startTime).getTime()) / 1000
                  : null,
              }));

            const testStage = records.find((r: any) =>
              r.startTime && r.finishTime && (
                /test/i.test(r.name) ||
                r.type === "Task" && /VsTest|DotNetCoreCLI|Maven|Gradle|pytest|jest|mocha|xunit|nunit/i.test(r.task?.name || "")
              )
            );
            if (testStage && testStage.startTime && testStage.finishTime) {
              const testMins =
                (new Date(testStage.finishTime).getTime() - new Date(testStage.startTime).getTime()) / 60000;
              buildEntry.testExecutionMins = Math.round(testMins * 10) / 10;
              totalTestMins += testMins;
              testBuildCount++;
            } else {
              const testRun = await fetchTestRunForBuild(buildEntry.id, base, pipelinesHeaders);
              if (testRun !== null) {
        ...
        if (testBuildCount > 0) {
          avgTestExecutionMins = Math.round((totalTestMins / testBuildCount) * 10) / 10;
        }
      ...
      } catch (e: any) { console.log("[collectMetrics] build error:", e?.message); }
    }

    let bugCount = 0, bugsOpen = 0, storiesCompleted = 0, tasksCompleted = 0, avgCycleTimeDays = 0;
    ...
    try {
      ...
      const openBugRes = await axios.post(
        `${base}/_apis/wit/wiql?api-version=7.1`,
        { query: `SELECT [System.Id] FROM WorkItems WHERE [System.TeamProject] = '${adoProject}' AND [System.WorkItemType] = 'Bug' AND [System.State] <> 'Closed' AND [System.State] <> 'Resolved'` },
        { headers: workItemsHeaders }
      );
      ...
      console.log(`[collectMetrics] Work items: bugs=${bugCount} open=${bugsOpen} closedItems=${allItemIds.length}`);

      for (let i = 0; i < allItemIds.length; i += 200) {
        const batchIds = allItemIds.slice(i, i + 200).map((w: any) => w.id).join(",");
        try {
          ...
          for (const item of (detailRes.data.value || [])) {
            const f = item.fields;
            const rawAssignee: string = f["System.AssignedTo"]?.displayName || "Unassigned";
            ...
            if (!workItemsByDev[assignee]) workItemsByDev[assignee] = { completed: 0, totalCycleDays: 0, bugs: 0, stories: 0, tasks: 0 };
            workItemsByDev[assignee].completed++;
            if (workItemType === "Bug") workItemsByDev[assignee].bugs++;
            else if (workItemType === "User Story") { workItemsByDev[assignee].stories++; storiesCompleted++; }
            else if (workItemType === "Task") { workItemsByDev[assignee].tasks++; tasksCompleted++; }
            if (created && closed) {
              const days = (new Date(closed).getTime() - new Date(created).getTime()) / 86400000;
              totalCycleDays += days; cycleCount++;
              workItemsByDev[assignee].totalCycleDays += days;
            }
          }
          if (cycleCount > 0) avgCycleTimeDays = Math.round((totalCycleDays / cycleCount) * 10) / 10;
        } catch (e: any) { console.log("[collectMetrics] work items batch error:", e?.message); }
      }
    } catch (e: any) { console.log("[collectMetrics] work items error:", e?.message); }
    ...
    for (const [k, v] of Object.entries(developerPrMap)) {
      ...
      if (!mergedDevs[n]) mergedDevs[n] = { prCount: 0, totalReviewHours: 0, comments: 0, commits: 0, completed: 0, totalCycleDays: 0, bugs: 0, stories: 0, tasks: 0, totalComments: 0 };
      mergedDevs[n].prCount += v.prCount;
      mergedDevs[n].totalReviewHours += v.totalReviewHours;
      mergedDevs[n].comments += v.comments;
      mergedDevs[n].totalComments += v.comments;
    }
    ...
    const developerMetrics = Object.entries(mergedDevs).map(([name, d]) => ({
      name,
      prCount: d.prCount,
      avgPrReviewHours: d.prCount > 0 ? Math.round((d.totalReviewHours / d.prCount) * 10) / 10 : 0,
      ...
      avgCycleDays: d.completed > 0 ? Math.round((d.totalCycleDays / d.completed) * 10) / 10 : 0,
    })).sort((a, b) => (b.commits + b.prCount + b.workItemsCompleted) - (a.commits + a.prCount + a.workItemsCompleted));

    const deploymentsLast30 = buildLog.filter((b) =>
      b.result === "succeeded" && b.startTime &&
      Date.now() - new Date(b.startTime).getTime() < 30 * 86400000
    ).length;
    const deployFrequencyPerDay = Math.round((deploymentsLast30 / 30) * 100) / 100;

    const payload: any = {
      avgReviewTimeHours, prCount, avgCommentsPerPr, totalPrComments, avgPrPickupHours,
      successRate, avgBuildTime, coverage, bugCount, bugsOpen,
      totalCommits, uniqueContributors, commitsByAuthor,
      longestBuildMins: Math.round(longestBuildMins * 10) / 10,
      shortestBuildMins: shortestBuildMins === Infinity ? 0 : Math.round(shortestBuildMins * 10) / 10,
      buildsByResult, storiesCompleted, tasksCompleted, avgCycleTimeDays,
      deploymentsLast30, deployFrequencyPerDay,
      buildLog, prLog, developerMetrics,
      updatedAt: new Date(),
    };
    ...
    res.status(200).json({
      message: "metrics updated", docId,
      summary: { prCount, successRate, avgBuildTime, coverage, bugCount, totalCommits, avgTestExecutionMins, developerCount: developerMetrics.length },
    });
  } catch (err: any) {
  ...
  try {
    ...
    for (const run of runs) {
      if (run.startedDate && run.completedDate) {
        const mins =
          (new Date(run.completedDate).getTime() - new Date(run.startedDate).getTime()) / 60000;
        totalMins += mins;
        count++;
      }
    }
...
async function handleGithubRepo(req: any, res: any, opts: {
  docId: string; githubFullName: string; githubPat: string;
  pipelineId?: string; org?: string; adoProject?: string;
  headers?: any; fromDate?: string; toDate?: string;
}) {
  const { docId, githubFullName, githubPat, pipelineId, fromDate, toDate } = opts;
  const org = opts.org || "";
  ...
  if (!githubFullName || !githubPat) {
    return res.status(400).json({ error: `GitHub repo requires githubFullName and x-github-pat. fullName="${githubFullName}"` });
  }

  console.log(`[collectMetrics/github] fetching PRs+commits for ${githubFullName}`);
  const [prs, commits] = await Promise.all([getGithubPRs(githubFullName, githubPat), getGithubCommits(githubFullName, githubPat)]);
  console.log(`[collectMetrics/github] PRs=${prs.length} commits=${commits.length}`);

  const fromMs = fromDate ? new Date(fromDate).getTime() : 0;
  const toMs = toDate && toDate !== "now" ? new Date(toDate).getTime() : Date.now();
  ...
  for (const pr of prs) {
    ...
    const hrs = closedAt ? (new Date(closedAt).getTime() - created) / 3600000 : (Date.now() - created) / 3600000;
    if (closedAt) totalReviewHours += hrs;
    const author: string = pr.user?.login || "Unknown";
    if (!developerPrMap[author]) developerPrMap[author] = { prCount: 0, totalReviewHours: 0 };
    developerPrMap[author].prCount++;
    developerPrMap[author].totalReviewHours += hrs;
    prLog.push({
      id: pr.number, title: pr.title, author,
      reviewTimeHours: closedAt ? Math.round(hrs * 10) / 10 : null,
      comments: (pr.comments || 0) + (pr.review_comments || 0),
      createdDate: pr.created_at, closedDate: closedAt || null,
  ...
  const avgReviewTimeHours = closedPrCount > 0 ? Math.round((totalReviewHours / closedPrCount) * 10) / 10 : 0;

  const commitsByAuthor: Record<string, number> = {};
  for (const c of commits) {
    const commitDate = new Date(c.commit?.author?.date || c.commit?.committer?.date || 0).getTime();
  ...
  let successRate = 0, avgBuildTime = 0, coverage: number | null = null;
  let longestBuildMins = 0, shortestBuildMins = Infinity, shortMin = Infinity;
  let avgTestExecutionMins: number | null = null;
  const buildLog: any[] = [];
  const buildsByResult: Record<string, number> = {};
  let deploymentsLast30 = 0, deployFrequencyPerDay = 0;

  if (pipelineId && pipelineId !== "none" && adoHeaders && org && adoProject) {
    const adoBase = `https://dev.azure.com/${org}/${encodeURIComponent(adoProject)}`;
    try {
      const buildRes = await axios.get(
      ...
      for (const b of builds) {
        ...
        if (b.startTime && b.finishTime) {
          ...
          totalTime += durationMins;
          if (durationMins > longestBuildMins) longestBuildMins = durationMins;
          if (durationMins < shortMin) shortMin = durationMins;
        }
        buildLog.push({
          id: b.id, buildNumber: b.buildNumber, result, status: b.status,
          startTime: b.startTime, finishTime: b.finishTime,
      ...
      if (builds.length > 0) {
        successRate = Math.round((success / builds.length) * 100);
        avgBuildTime = Math.round((totalTime / builds.length) * 10) / 10;
        shortestBuildMins = shortMin === Infinity ? 0 : Math.round(shortMin * 10) / 10;
        longestBuildMins = Math.round(longestBuildMins * 10) / 10;
      }
      ...
      for (let i = 0; i < buildLog.length; i++) {
        ...
        try {
          ...
          if (testStage && testStage.startTime && testStage.finishTime) {
            ...
            const testRun = await fetchTestRunForBuild(buildEntry.id, adoBase, adoHeaders);
            if (testRun !== null) {
              buildEntry.testExecutionMins = testRun;
              totalTestMins += testRun;
              testBuildCount++;
      ...
      deploymentsLast30 = buildLog.filter((b) =>
        b.result === "succeeded" && b.startTime &&
        Date.now() - new Date(b.startTime).getTime() < 30 * 86400000
      ).length;
      ...
      for (const b of builds.slice(0, 5)) {
        try {
          ...
          for (const stat of (covRes.data?.coverageData?.[0]?.coverageStats || [])) {
            if (stat.label === "Lines" && stat.total > 0) { coverage = Math.round((stat.covered / stat.total) * 100); break; }
          }
          if (coverage !== null) break;
        } catch (e) { }
      }
    } catch (e: any) { console.log("[collectMetrics/github] ADO pipeline error:", e?.message); }
  }

  let bugCount = 0, bugsOpen = 0, storiesCompleted = 0, tasksCompleted = 0, avgCycleTimeDays = 0;

  if (adoHeaders && org && adoProject) {
    const adoBase = `https://dev.azure.com/${org}/${encodeURIComponent(adoProject)}`;
    const wiqlDateFilter = fromDate
    ...
    try {
      ...
      for (let i = 0; i < allItemIds.length; i += 200) {
        ...
        try {
          ...
          for (const item of (detailRes.data.value || [])) {
            const f = item.fields;
            const assignee: string = f["System.AssignedTo"]?.displayName || "Unassigned";
            const workItemType: string = f["System.WorkItemType"] || "Unknown";
            const created = f["System.CreatedDate"];
            const closed = f["Microsoft.VSTS.Common.ClosedDate"];
            if (!workItemsByDev[assignee]) workItemsByDev[assignee] = { completed: 0, totalCycleDays: 0, bugs: 0, stories: 0, tasks: 0 };
      ...
      console.log(`[collectMetrics/github] work items: bugs=${bugCount} open=${bugsOpen}`);
    } catch (e: any) { console.log("[collectMetrics/github] work items error:", e?.message); }
  }

  const mergedDevs: Record<string, any> = {};
  for (const [k, v] of Object.entries(developerPrMap)) {
    if (!mergedDevs[k]) mergedDevs[k] = { prCount: 0, totalReviewHours: 0, commits: 0 };
  ...
  const payload: any = {
    prCount: prs.length, avgReviewTimeHours, avgCommentsPerPr: 0, totalPrComments: 0, avgPrPickupHours: 0,
    successRate, avgBuildTime, coverage, bugCount, bugsOpen,
    storiesCompleted, tasksCompleted, avgCycleTimeDays,
    totalCommits: Object.values(commitsByAuthor).reduce((a, b) => a + b, 0),
    uniqueContributors: Object.keys(commitsByAuthor).length,
    commitsByAuthor, longestBuildMins, shortestBuildMins,
    buildsByResult, deploymentsLast30, deployFrequencyPerDay,
    buildLog, prLog, developerMetrics, updatedAt: new Date(), source: "github",
  };
  ...
  return res.status(200).json({
    message: "github metrics updated", docId,
    summary: { prCount: prs.length, totalCommits: payload.totalCommits, builds: buildLog.length, avgTestExecutionMins },
  });
}
